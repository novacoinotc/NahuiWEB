...
const navLinks = [
  { href: "#que-hacemos", label: "Soluciones" },
  { href: "#diferenciadores", label: "Por que nosotros" },
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex items-center justify-between py-4">
          <a href="#" className="flex items-center gap-3 group" aria-label="Nahui Labs - Inicio">
            <div className={`transition-all duration-500 ${isScrolled ? "drop-shadow-[0_0_8px_rgba(0,229,255,0.4)]" : ""}`}>
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-white hover:text-[#00E5FF] transition-colors duration-300 text-sm font-medium font-body relative group"
              >
                {link.label}
                ...
                <div className="flex flex-col gap-4">
                  {navLinks.map((link, index) => (
                    <motion.a
                      ...
                      className="text-white hover:text-[#00E5FF] transition-colors duration-300 text-base font-medium py-2 font-body"
                    >
                      {link.label}
                    </motion.a>
                  ))}
